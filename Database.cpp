...
Database::Database(ofstream * pf){
	pfichier = pf;
	indexInformation();
}

string Database::getHeaderFileName() const{
	return headerFileName;
}

string Database::getSequenceFileName() const{
	return sequenceFileName;
}

string Database::getIndexFileName() const{
...
void Database::indexInformation(){ // récupérer les informations dans l'index
	...
	if (indexFile.is_open()){
		...
		cout << "You do not have the right index file name. The name of the file should be " << indexFileName << endl;
	}
...
void Database::printSequenceName (int * places, int * scores){
	...
	*pfichier << "DISPLAY OF THE 50 BEST ALIGNEMENTS : " << endl << endl;
	...
	if (headerFile.is_open()){
		...
		cout << "You do not have the right header file name. The name of the file should be " << headerFileName << endl;
	}
