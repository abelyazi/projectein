...
int Algorithm::calculateScore(int * querySequence, int sequenceSize, int sequenceSize2, int * databaseSequence){
...
void Algorithm::calculate(Sequence * sequence, Database * database){
	...
	if (sequenceFile.is_open()){
		for (unsigned int p = 0; p<sequenceOffsetTable.size()-1; p++){
			...
			int score = calculateScore(querySequence, querySequenceSize, databaseSequenceSize, databaseSequence);
		...
		cout << "You do not have the right sequence file name. The name of the file should be " << database->getName() + ".psq" << endl;
	}
	sequenceFile.close();
